  ...
  if (!resolvedCompanyId) {
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">{t("pluginPage.selectCompanyToView")}</p>
      </div>
    );
  }
    ...
    if (duplicateMatches.length > 1) {
      return (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          {t("pluginPage.multiplePluginsDeclareRoute")} <code>{pluginRoutePath}</code>. {t("pluginPage.usePluginIdRoute")}
        </div>
      );
    }
  }
