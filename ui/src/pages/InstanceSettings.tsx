...
export function InstanceSettings() {
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("settings.loadingHeartbeats")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("settings.failedToLoadHeartbeats")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("settings.schedulerHeartbeats")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("settings.heartbeatsDesc")}
        </p>
      </div>
      ...
      <div className="flex gap-4 text-sm text-muted-foreground">
        ...
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? t("settings.company") : t("settings.company")}</span>
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("settings.noHeartbeats")}
        />
      ) : (
