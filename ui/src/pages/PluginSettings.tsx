...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? t("common.detail") },
    ]);
  ...
  if (pluginLoading) {
    return <div className="p-4 text-sm text-muted-foreground">{t("plugins.loadingPluginDetails")}</div>;
  }
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "configuration" | "status")} className="space-y-6">
        <PageTabBar
          ...
          items={[
            ...
            { value: "status", label: t("plugins.pluginStatus") },
          ]}
          value={activeTab}
          ...
          <div className="space-y-8">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">{t("plugins.none")}</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <section className="space-y-4">
              <div className="space-y-1">
                <h2 className="text-base font-semibold">{t("plugins.settings")}</h2>
              </div>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {t("plugins.workerProcess")}, scheduled jobs, and webhook deliveries
                  </CardDescription>
                </CardHeader>
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  ...
                                  <span className="text-xs">
                                    {dashboardData.worker.consecutiveCrashes} {t("plugins.consecutive")} / {dashboardData.worker.totalCrashes} {t("plugins.total")}
                                  </span>
                                </div>
              ...
              {recentLogs && recentLogs.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>{t("plugins.logEntries").replace("{count}", String(recentLogs.length))}</CardDescription>
                  </CardHeader>
                  <CardContent>
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      setSaveMessage({ type: "success", text: t("plugins.configurationSaved") });
      setTestResult(null);
      queryClient.invalidateQueries({ queryKey: queryKeys.plugins.config(pluginId) });
      setTimeout(() => setSaveMessage(null), 3000);
    },
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("plugins.saveFailed") });
    },
  });
