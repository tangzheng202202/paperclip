  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        ...
        body: t("routines.addFirstTrigger"),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : (isZhCN ? "无法更新流程。" : "Paperclip could not update the routine."),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : (isZhCN ? "无法启动流程运行。" : "Paperclip could not start the routine run."),
        tone: "error",
      });
    },
  });
                    ...
                    >
                      ...
                      <SelectContent>
                        {concurrencyPolicies.map((value) => (
                          <SelectItem key={value} value={value}>{isZhCN ? t(`routines.${value}`) : value.replaceAll("_", " ")}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                    <p className="text-xs text-muted-foreground">{isZhCN ? t(`routines.${draft.concurrencyPolicy}`) : (draft.concurrencyPolicy === "coalesce_if_active" ? "If a run is already active, keep just one follow-up run queued." : draft.concurrencyPolicy === "always_enqueue" ? "Queue every trigger occurrence, even if the routine is already running." : "Drop new trigger occurrences while a run is still active.")}</p>
                  </div>
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">{isZhCN ? t(`routines.${draft.catchUpPolicy}`) : (draft.catchUpPolicy === "skip_missed" ? "Ignore windows that were missed while the scheduler or routine was paused." : "Catch up missed schedule windows in capped batches after recovery.")}</p>
                  </div>
                </div>
              </CollapsibleContent>
            </Collapsible>
