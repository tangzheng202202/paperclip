    ...
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {agents.length} 个{t("companyImport.agent")}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
...
async function readLocalPackageZip(file: File): Promise<{
  ...
  if (Object.keys(archive.files).length === 0) {
    throw new Error("在选定的 zip 存档中未找到包文件。");
  }
  return {
    name: file.name,
...
export function CompanyImport() {
  ...
  const localZipHelpText =
    "上传直接从 Paperclip 导出的 .zip 文件。由 Finder、Explorer 或其他压缩工具重新压缩的档案可能无法正确导入。";
  ...
  const previewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        ...
        body: err instanceof Error ? err.message : t("companyImport.noSourceConfigured"),
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        ...
        body: `${result.company.name}: ${result.agents.length} 个员工已处理。`,
      });
    ...
    onError: (err) => {
      pushToast({
        ...
        body: err instanceof Error ? err.message : t("companyImport.importFailed"),
      });
    },
  });
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      pushToast({
        ...
        body: err instanceof Error ? err.message : t("companyImport.packageReadFailed"),
      });
    }
  }
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message={t("companyImport.selectCompanyToImport")} />;
  }
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        ...
        {sourceMode === "local" ? (
          ...
          <Field
            ...
            hint="仓库树路径或 COMPANY.md 的 blob URL（例如 github.com/owner/repo/tree/main/company）。"
          >
            <input
        ...
        <Field label={t("companyImport.target")} hint={t("companyImport.targetDesc")}>
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
