    ...
    <div className="space-y-6">
      <div className="space-y-5">
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              ...
              subtitle={`${formatTokens(inferenceTokenTotal)} tokens（请求范围内）`}
              icon={DollarSign}
            />
            <MetricTile
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? `${budgetData?.pausedAgentCount ?? 0} 个员工已暂停 · ${budgetData?.pausedProjectCount ?? 0} 个项目已暂停`
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
            ...
            <MetricTile
              ...
              subtitle={`${formatCents(financeData?.summary.debitCents ?? 0)} 支出 · ${formatCents(financeData?.summary.creditCents ?? 0)} 收入`}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("costs.financeEvents")}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={`${formatCents(financeData?.summary.estimatedDebitCents ?? 0)} 预估`}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          ...
          <TabsTrigger value="budgets">{t("costs.budget")}</TabsTrigger>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          已消耗本月 {spendData.summary.utilizationPercent}% 的预算。
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    ...
                    <CardDescription>所选期间每个员工的消耗。</CardDescription>
                  </CardHeader>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>通过项目关联任务归因的运行成本。</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  ...
                  <CardDescription>
                    员工和项目的硬停止消费限制。提供商订阅配额保持独立，显示在提供商下。
                  </CardDescription>
                </CardHeader>
