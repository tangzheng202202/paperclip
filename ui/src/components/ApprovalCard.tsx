...
export function ApprovalCard({
  ...
  const label = approvalLabel(approval.type, approval.payload as Record<string, unknown> | null, isZhCN ? t : undefined);
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      ...
      case "revision_requested": return t("approval.revisionRequested");
    ...
    <div className="border border-border rounded-lg p-4 space-y-0">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <span className="text-xs text-muted-foreground capitalize">{getStatusLabel(approval.status)}</span>
