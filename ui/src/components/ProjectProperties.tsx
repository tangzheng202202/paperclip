...
const PROJECT_STATUSES = [
  { value: "backlog", labelKey: "projectStatus.backlog" },
  { value: "planned", labelKey: "projectStatus.planned" },
  { value: "in_progress", labelKey: "projectStatus.inProgress" },
  { value: "completed", labelKey: "projectStatus.completed" },
  { value: "cancelled", labelKey: "projectStatus.cancelled" },
];
...
function SaveIndicator({ state, t }: { state: ProjectFieldSaveState; t: (key: string) => string }) {
  if (state === "saving") {
    return (
...
function ProjectStatusPicker({ status, onChange, t }: { status: string; onChange: (status: string) => void; t: (key: string) => string }) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {t(`projectStatus.${status.replace("_", "")}`) || status.replace("_", " ")}
        </button>
      </PopoverTrigger>
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const submitLocalWorkspace = () => {
    ...
    if (!isAbsolutePath(cwd)) {
      setWorkspaceError(t("projectProperties.localFolderMustBeAbsolute"));
      return;
    }
    setWorkspaceError(null);
    persistCodebase({ cwd });
  ...
  const submitRepoWorkspace = () => {
    ...
    if (!isGitHubRepoUrl(repoUrl)) {
      setWorkspaceError(t("projectProperties.mustBeValidGitHubRepo"));
      return;
    }
    setWorkspaceError(null);
  ...
  const clearLocalWorkspace = () => {
    const confirmed = window.confirm(
      codebase.repoUrl
        ...
        : t("projectProperties.deleteWorkspaceLocal"),
    );
    if (!confirmed) return;
    persistCodebase({ cwd: null });
  };
  ...
  const clearRepoWorkspace = () => {
    ...
    const confirmed = window.confirm(
      hasLocalFolder
        ...
        : t("projectProperties.deleteWorkspaceRepo"),
    );
    if (!confirmed) return;
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        <PropertyRow label={<FieldLabel label={t("projectProperties.name")} state={fieldState("name")} t={t} />}>
        ...
        <PropertyRow
          label={<FieldLabel label={t("projectProperties.description")} state={fieldState("description")} t={t} />}
          alignStart
        ...
        <PropertyRow label={<FieldLabel label={t("projectProperties.status")} state={fieldState("status")} t={t} />}>
          {onUpdate || onFieldUpdate ? (
            <ProjectStatusPicker
        ...
        <PropertyRow
          label={<FieldLabel label={t("projectProperties.goals")} state={fieldState("goals")} t={t} />}
          alignStart
        ...
        <PropertyRow label={<FieldLabel label={t("projectProperties.created")} state="idle" t={t} />}>
          <span className="text-sm">{formatDate(project.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("projectProperties.updated")} state="idle" t={t} />}>
          <span className="text-sm">{formatDate(project.updatedAt)}</span>
        </PropertyRow>
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label={t("projectProperties.targetDate")} state="idle" t={t} />}>
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                ...
                <Tooltip>
                  ...
                  <TooltipContent side="top">
                    {isZhCN ? "隔离任务检出和执行工作区行为的项目级默认值。" : "Project-owned defaults for isolated issue checkouts and execution workspace behavior."}
                  </TooltipContent>
                </Tooltip>
              </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      ...
                      <SaveIndicator state={fieldState("execution_workspace_enabled")} t={t} />
                    </div>
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>{t("projectProperties.newIssuesIsolated")}</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} t={t} />
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          {t("projectProperties.newIssuesIsolatedDesc")}
                        </div>
                      </div>
                      <button
                        className={cn(
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          {t("projectProperties.hostManaged")} <span className="text-foreground">{t("projectProperties.gitWorktree")}</span>
                        </div>
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("projectProperties.baseRef")}</span>
                              <SaveIndicator state={fieldState("execution_workspace_base_ref")} t={t} />
                            </label>
                          </div>
                          <DraftInput
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              ...
                              <SaveIndicator state={fieldState("execution_workspace_branch_template")} t={t} />
                            </label>
                          </div>
                          <DraftInput
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              ...
                              <SaveIndicator state={fieldState("execution_workspace_worktree_parent_dir")} t={t} />
                            </label>
                          </div>
                          <DraftInput
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              ...
                              <SaveIndicator state={fieldState("execution_workspace_provision_command")} t={t} />
                            </label>
                          </div>
                          <DraftInput
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              ...
                              <SaveIndicator state={fieldState("execution_workspace_teardown_command")} t={t} />
                            </label>
                          </div>
                          <DraftInput
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          {t("projectProperties.provisionDesc")}
                        </p>
                      </div>
                    ) : null}
                  </div>
